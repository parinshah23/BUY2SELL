...
export default function Navbar() {
  ...
  // Floating Navbar Code - Hidden as requested
  /*
  const width = useTransform(scrollY, [0, 500], ["100%", "95%"]);
  const marginTop = useTransform(scrollY, [0, 500], ["0px", "10px"]);
  const borderRadius = useTransform(scrollY, [0, 500], ["0px", "20px"]);
  const backgroundColor = useTransform(scrollY, [0, 500], ["rgba(255, 255, 255, 1)", "rgba(255, 255, 255, 0.95)"]);
  const backdropFilter = useTransform(scrollY, [0, 500], ["blur(0px)", "blur(10px)"]);
  const border = useTransform(scrollY, [0, 500], ["1px solid rgba(0,0,0,0)", "1px solid rgba(0,0,0,0.1)"]);
  const paddingY = useTransform(scrollY, [0, 500], ["0px", "0px"]);
  */
  ...
  return (
    ...
    <header className="fixed top-0 left-0 right-0 z-50 w-full bg-gradient-to-br from-primary-50 to-secondary-100 shadow-sm border-b border-secondary-100 flex flex-col">
      {/* 
    <div className="fixed top-0 left-0 right-0 z-50 flex justify-center pointer-events-none">
      <motion.header
        style={{ width, marginTop, borderRadius, backgroundColor, backdropFilter, border }}
    ...
    */}
      ...
      {/* 🔻 Mega Menu Category Bar (New) */}
      <div className="w-full hidden md:block border-t border-secondary-100">
        <CategoryNav />
      </div>
